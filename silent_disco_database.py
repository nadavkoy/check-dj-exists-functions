...
class Database(object):
    ...
    def username_taken(self, username):
        ...
        for user in self.users_cursor:  # iterating the Users table
            if user[0] == username:  # check if the wanted username already exists
                return True
        return False
    ...
    def add_dj(self, username, password):
        """ given a username and a password, adds them to the DJs database if not taken. """
        if not self.dj_name_taken(username):
            self.djs_cursor.execute("INSERT INTO users VALUES(?, ?)", (username, password))
            self.conn.commit()
            return True  # user added successfully.

        return False  # username was taken. Therefore, user couldn't be added.

    def dj_name_taken(self, username):
        """ returns 'true' if username is already taken, 'false' otherwise. """
        self.djs_cursor.execute('select * from djs')
        for dj in self.djs_cursor:  # iterating the Users table
            if dj[0] == username:  # check if the wanted username already exists
                return True
        return False

    def dj_exists(self, username, password):
        """ checks if the username and password entered actually belong to an existing dj. """
        self.djs_cursor.execute('select * from djs')
        for dj in self.djs_cursor:  # iterating the Users table
            if dj[0] == username and dj[1] == password:  # check if the details match any of the the existing ones.
                return True
        return False  # there is no such user with such password. therefore, return False.
